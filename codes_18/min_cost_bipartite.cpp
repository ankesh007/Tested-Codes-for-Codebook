// Min cost bipartite matching via shortest augmenting path
// This is an O(n^3) implementation of a shortest augmenting path
// algorithm for finding min cost perfect matchings in dense
// graphs. In practice, it solves 1000x1000 problems in around 1 second.
...
long MinCostMatching(const VVD &cost, VI &Lmate, VI &Rmate) {
	...
	while (mated < n) {			// find an unmatched left node
		int s = 0;
		while (Lmate[s] != -1) s++;			// initialize Dijkstra
		...
		while (true){				// find closest
			...
			if (Rmate[j] == -1) break ;				// relax neighbors
			const int i = Rmate[j] ;
			for (int k = 0; k < n; k++) {
				if (seen[k]) continue;
