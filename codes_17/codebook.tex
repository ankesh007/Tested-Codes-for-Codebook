% \date{\today}
\date{}
\title{\vspace{-1cm}Codebook- Team Know\_no\_algo\\IIT Delhi, India}
...
\lstset{
    tabsize=1,
    basicstyle=\fontsize{11}{8}\ttfamily,
...
\subsection{Format Java} % Stanford
\lstinputlisting[language=java]{Format.java}
...
\lstinputlisting[language=java]{Manacher.java}

\subsection{Suffix\_Array} % Stanford
\lstinputlisting[language=java]{Suffix_Array.java}

\subsection{Z algo} % Stanford
\lstinputlisting[language=java]{Z_algo.java}

\subsection{Hashing} % Stanford
\lstinputlisting[language=c++]{hash.cpp}

\subsection{Trie} % Stanford
...
\subsection{Ford Fulkerson Matching} % Stanford
...
\subsection{Primi Root } % Stanford
...
\subsection{Order Stat Tree} % Brian Bi
